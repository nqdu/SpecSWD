 ...
 */
static int 
find_loc(const Real *z,Real z0,int nz)
{
 ...
 */
void Mesh:: 
interp_model(const Real *param,const std::vector<int> &elmnts,std::vector<Real> &md) const
{
        ...
        for(int ispec_md = 0; ispec_md < nel; ispec_md ++) {
            ...
            for(int i = 0; i < NGL; i ++) {
                ...
                if(j < istart || j >= iend) {
                    ...
                    Real dzinv = 1./ (depth_tomo[j + 1] - depth_tomo[j]);
 ...
 */
void Mesh:: 
project_kl(const Real *frekl, Real *kl_out) const
{
    using GQTable :: NGLL; using GQTable :: NGRL;
            ...
            for(int ispec_md = 0; ispec_md < nel; ispec_md ++) {
                ...
                for(int i = 0; i < NGL; i ++) {
                    ...
                    if(j < istart || j >= iend) {
                        ...
                        Real dz = depth_tomo[j + 1] - depth_tomo[j];
                        Real coef = (z0 - depth_tomo[j]) / dz;
